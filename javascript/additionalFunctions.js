...
$(document).ready(function () {
	...
	// Calendar closes if it loose focus
    ...
    // displaying tips above the mode icons on hover, if it is not mobile
        ...
        switch (selectedMode) {
            ...
            case 'BUSISH%2CWALK':
                if ($(".now-later, .walkSpeed, .walkDistance").is(':hidden')) {
                    ...
                    $(".walkSpeed, .walkDistance").hide('fast');
                break;
...
function CO2calculation(carDistance, busDistance) {
    ...
    if (selectedMode == 'WALK' || selectedMode == 'BICYCLE')
        ...
        // If car is electric, co2/km = 0g.
        if($('#fuelTypeInput').val() == 0) {
            totalCO2 = 0 * carDistanceKm + busEmission;
        }
        else if (co2_per_km != "")
            ...
            // Default CO2 emission/km = 180g.
            totalCO2 = 180 * carDistanceKm + busEmission;
    }
