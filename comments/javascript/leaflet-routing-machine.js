var departure0, departure1, departure2;
var distBeforeBus = 0;
var isBeforeBus = true;
var speedOfWalking;
	...
	L.Routing.Itinerary = L.Control.extend({
		...
		setAlternatives: function(routes) {
			...
			for (i = 0; i < this._routes.length; i++) {
				...
				if(departure0 != null)
				{
				// Calculating time needed to reach bus
				switch(speedOfWalking)
				{
			...
			if(departure0 != null)
			{
			// if minutes less than 10 adding extra 0 before the number to avoid this 15:8 format
			splitted = departure0.split(':');
			...
			if(departure1 != "NaN:NaN")
				$("#busTime1").text(departure1);
			if(departure2 != "NaN:NaN")
				$("#busTime2").text(departure2);

			// To reuse variable we have to assign them back to null
			departure0 = undefined;
			departure1 = undefined;
			departure2 = undefined;
			distBeforeBus = 0;
		...
		_createAlternative: function(alt, i) {
            ...
            /* this._clickInstructions(header);
			this._backToAlts(header); */
		...
		_createItineraryContainer: function(r, collapseIndex) {
			...
			if (collapseIndex == 0) {			// before it was 1
				k = 10;
			}
			else if (collapseIndex == 1) {		// before it was 2
				k = 20;
			}
			else if (collapseIndex == 2) {		//before it was 3
				k = 30;
			}
			...
			for (j = 0; j < r.coordinates.length; j++) {
				for (i = 0; i < r.instructions[j].length; i++) {
					...
					if (instr.type === 'busStop') {
						...
						step = this._itineraryBuilder.createBusStop(stopID, stopName, time, panelBody, collapseIndex);
						...
						if (instr.type === 'INIT') {
							...
							if (instr.mode === 'BUS') {
								...
								stopID = instr.stopIndex;
								panelTitle = this._itineraryBuilder.createPanelTitleBus(text, k, icon, time, busNumber, stopID, true, collapseIndex);
							}
							...
							// If new leg is other mode than bus there are two oprions
								...
								if (busFlag) {
									...
									time = instr.timeArr;
									stopID = instr.stopIndex;
									panelTitle = this._itineraryBuilder.createPanelTitleBus(text, k, icon, time, busNumber, stopID, false, false);
								}
								// If new leg doesn't go after bus, it has
         ...
         * @private
         */
		 _doubleTapHeader: function(header){
...
(function() {
	...
	L.Routing.ItineraryBuilder = L.Class.extend({
		...
		createPanelTitleBus: function (text, k, icon, time, busNumber, stopID, i, collapseIndex) {
			var panelTitle = L.DomUtil.create('h4', 'panel-title'),
				header = L.DomUtil.create('a', '', panelTitle);
			header.setAttribute('data-toggle', 'collapse');
			header.setAttribute('data-parent', '#accordion');
		...
		createBusStop: function(stopID, stopName, time, panelBody, i) {
			var step = L.DomUtil.create('ul', '', panelBody),
			...
			for (var j = 0; j < route.coordinates.length; j++) {
				...
				if (route.instructions[j][0] != undefined){
					if (route.instructions[j][0]["mode"] == "WALK"){
						...
						this.options.styles[2]["color"] = "#00FFFF";
						this.options.styles[2]["weight"] = 5;
					}
...
(function() {
	...
	L.Routing.OSRM = L.Class.extend({
		...
		/**
		 * Function builds the URL - request which is sent to the server
		 * @param waypoints
		 * @param options
         * @returns {string}
         */
